...
-   A new field named ‘**upgradCoins**’ will get added as an attribute. ‘upgradCoins’ is a kind of currency that can be used by each user to purchase property on the asset. For example, if the price of a property is stated as 500 upgradCoins, only a user with ‘upgradCoins’ equal to or greater than 500 will be able to purchase it. The initial value of this attribute is set to zero. There is a separate function to recharge the account balance which you will see next.  
